...
/* Foundation Class 4 is for SUB-BASE only 
 * Hsb min = 200mm
...
public class FdnClass4 {
	...
	FdnClass4(){
		...
		System.out.println("Enter sub-base modulus of elasticity 'Esb' between 1000MPa and 3500MPa: ");
	...
	public double HsbCalc(double EsbVar, double cbrVar) {
		...
		Hsb = 1.53*Math.pow(10, 4) * Math.pow(EsbVar, -0.4833) * (1 - 0.234*Math.pow(EsbVar, -0.025)*Math.log(cbrVar));
	...
	public void calculate() {
		...
		if(this.cbr >= 2.5 && this.cbr <= 30) {
			...
			System.out.println("CBR value out of range");
